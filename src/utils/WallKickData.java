...
public class WallKickData {
  ...
  private static final int[][] DEFAULT_UR_KICK = {
      { 0, 0 }, { -1, 0 }, { -1, 1 }, { 0, -2 }, { -1, -2 }
  };

  // NOTE: RIGHT to UP rotation
  private static final int[][] DEFAULT_RU_KICK = {
      { 0, 0 }, { 1, 0 }, { 1, -1 }, { 0, 2 }, { 1, 2 }
  };

  // NOTE: RIGHT to DOWN rotation
  private static final int[][] DEFAULT_RD_KICK = {
      { 0, 0 }, { 1, 0 }, { 1, -1 }, { 0, 2 }, { 1, 2 }
  };

  // NOTE: DOWN to RIGHT rotation
  private static final int[][] DEFAULT_DR_KICK = {
      { 0, 0 }, { -1, 0 }, { -1, 1 }, { 0, -2 }, { -1, -2 }
  };

  // NOTE: DOWN to LEFT rotation
  private static final int[][] DEFAULT_DL_KICK = {
      { 0, 0 }, { 1, 0 }, { 1, 1 }, { 0, -2 }, { 1, -2 }
  };

  // NOTE: LEFT to DOWN rotation
  private static final int[][] DEFAULT_LD_KICK = {
      { 0, 0 }, { -1, 0 }, { -1, -1 }, { 0, 2 }, { -1, 2 }
  };

  // NOTE: LEFT to UP rotation
  private static final int[][] DEFAULT_LU_KICK = {
      { 0, 0 }, { -1, 0 }, { -1, -1 }, { 0, 2 }, { -1, 2 }
  };

  // NOTE: UP to LEFT rotation
  private static final int[][] DEFAULT_UL_KICK = {
      { 0, 0 }, { 1, 0 }, { 1, 1 }, { 0, -2 }, { 1, -2 }
  };
  ...
  private static final int[][] ARIKA_I_UR_KICK = {
      { 0, 0 }, { -2, 0 }, { 1, 0 }, { 1, 2 }, { -2, -1 }
  };

  // NOTE: I Piece RIGHT to UP rotation
  private static final int[][] ARIKA_I_RU_KICK = {
      { 0, 0 }, { 2, 0 }, { -1, 0 }, { 2, 1 }, { -1, -2 }
  };

  // NOTE: I Piece RIGHT to DOWN rotation
  private static final int[][] ARIKA_I_RD_KICK = {
      { 0, 0 }, { -1, 0 }, { 2, 0 }, { -1, 2 }, { 2, -1 }
  };

  // NOTE: I Piece DOWN to RIGHT rotation
  private static final int[][] ARIKA_I_DR_KICK = {
      { 0, 0 }, { -2, 0 }, { 1, 0 }, { -2, 1 }, { 1, -1 }
  };

  // NOTE: I Piece DOWN to LEFT rotation
  private static final int[][] ARIKA_I_DL_KICK = {
      { 0, 0 }, { 2, 0 }, { -1, 0 }, { 2, 1 }, { -1, -1 }
  };

  // NOTE: I Piece LEFT to DOWN rotation
  private static final int[][] ARIKA_I_LD_KICK = {
      { 0, 0 }, { 1, 0 }, { -2, 0 }, { 1, 2 }, { -2, -1 }
  };

  // NOTE: I Piece LEFT to UP rotation
  private static final int[][] ARIKA_I_LU_KICK = {
      { 0, 0 }, { -2, 0 }, { 1, 0 }, { -2, 1 }, { 1, -2 }
  };

  // NOTE: I Piece UP to LEFT rotation
  private static final int[][] ARIKA_I_UL_KICK = {
      { 0, 0 }, { 2, 0 }, { -1, 0 }, { -1, 2 }, { 2, -1 }
  };
  ...
  private static Point2D getDefaultKickData(int rotationState, int direction, int kickIndex) {
    ...
    return new Point2D.Double(DEFAULT_KICKS[rotationState][directionIndex][kickIndex][0],
  ...
  private static Point2D getArikaKickData(int rotationState, int direction, int kickIndex) {
    int directionIndex = direction == RIGHT ? 0 : 1;
    return new Point2D.Double(ARIKA_KICKS[rotationState][directionIndex][kickIndex][0],
        ARIKA_KICKS[rotationState][directionIndex][kickIndex][1]);
  }

  public static Point2D getKickData(int shapeIndex, int rotationState, int direction, int kickIndex) {
    if (shapeIndex == I) {
